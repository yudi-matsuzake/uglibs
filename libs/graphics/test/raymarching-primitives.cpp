...
static constexpr auto vs = std::array{
R"__(
#version 330 core
...
/* static constexpr auto fs = std::array{ R"__( */
/* #version 330 */
...
/* void main() */
/* { */
/* 	color = vec4(1.f, 0.f, sin(u_time), 1.f); */
/* } */
/* )__"}; */

static constexpr auto fs = std::array{ R"__(
// code from www.shadertoy.com/view/Xds3zN 
...
class two_triangles : public ug::graphics::render {

	static constexpr auto scr_idx = std::array{
		0u, 1u, 2u, 0u, 2u, 3u
	};

public:
	explicit two_triangles(ug::graphics::app* app)
		: ug::graphics::render(app)
	{
		m_vertex_shader.set_source(vs);
		m_vertex_shader.compile();

		m_fragment_shader.set_source(fs);
		...
		m_vscr_vao.bind();
		m_vscr_ebo.set_data(scr_idx.data(), scr_idx.size());
		m_vscr_vao.bind();
		m_vscr_vbo.bind();
		m_vscr_vao.set_attribute_layout({ ug::graphics::vao::attr<float>(3) });
	}


	virtual ~two_triangles() = default;

	void draw()
	{
		auto const projection = m_app->compute_projection_matrix();
		auto const view = m_app->get_view_matrix();
		auto const pv = m_app->compute_projected_viewport(projection, view);

		auto const left	= pv.left();
		auto const right	= pv.right();
		auto const top	= pv.top();
		auto const bottom	= pv.bottom();
		...
		auto const [w, h] = m_app->get_framebuffer_size();
		auto const resolution = glm::vec2{
			static_cast<float>(w),
			static_cast<float>(h)
		};

		auto const mouse = m_app->get_cursor_vector();
		auto const time = static_cast<float>(m_app->get_time());
		...
		m_program.set_uniform("u_mouse", mouse);
		m_program.set_uniform("u_time", time);
...
private:
	ug::graphics::vertex_shader m_vertex_shader;
	ug::graphics::fragment_shader m_fragment_shader;
	ug::graphics::program m_program;

	ug::graphics::vao m_vscr_vao;
	ug::graphics::vbo m_vscr_vbo;
	ug::graphics::ebo m_vscr_ebo;
};

class raymarching_primitives : public ug::graphics::app {
public:
	raymarching_primitives()
		: ug::graphics::app(1280, 720, "raymarching primitives")
	{}

	virtual ~raymarching_primitives() = default;
...
private:
	...
	two_triangles render = two_triangles(this);
};

int main()
{
	raymarching_primitives app;
	return app.run();
}
